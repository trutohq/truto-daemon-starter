  ...
  async run() {
    ...
    for await (const order of orders) {
      if (order.paymentStatus == 'Paid') {
        ...
        const sageIntacctTaxDetails = (await this.trutoApi.proxyApi.create(
          {
            ...
            filters: [
              {
                $eq: {
                  'taxSolution.id': 'US Advanced Tax' /*order?.taxes[0].title*/,
                },
              },
            ],
          },
        ...
        const body = {
          ...
          lines: Object.entries(itemIds).map(([itemId, details]) => ({
            ...
            taxSolution: {
              id: sageIntacctTaxDetails[0].id,
              key: sageIntacctTaxDetails[0].key,
            },
            unit: 'Each',
