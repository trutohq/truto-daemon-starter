...
class Commerce7SageintacctOrders extends Job {
  async run() {
    ...
    for await (const order of ordersPromise) {
      if (order.purchaseType != 'Refund' && order.paymentStatus != 'Paid') {
        log.info(`Processing order ${order.id}`)
        const existingOrder = await this.checkExistingState(
        ...
        for (const item of commerce7Items) {
          ...
          warehouseIds[sageintacctItemId] =
            sageintacctWarehouseId as unknown as string
